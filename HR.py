...
llm = ChatDeepInfra(temperature=0.1)
...
Клиент: Здравствуйте! Я ищу курс для обучения построения нейросетей. Можете ли вы предложить мне подходящий вариант?
-Давай проверим, есть ли ключевое слово 'нейросеть' в названиях курсов: {course_names}? Нет.
-Тогда нам нужно искать либо в описании: {course_description}, либо в навыках: {course_skills}, либо в структуре курсов: {course_moduls}. Находим модуль 'Сверточные нейронные сети' в разделе модулей 'Course_program' курса 'Машинное обучение - продвинутый курс'. И отвечаем:
...
Если клиент попросит рассказать про продолжительность курса (приблизительное количество часов для прохождения) (для примера возьмём курс 'Машинное обучение'), то вам запрещено отвечать подобным образом:
