# import pandas as pd
# import numpy as np
# import json
# with open('course.json', encoding='utf-8') as f:
#     data = json.load(f)
#
# data = pd.DataFrame(data)
# def promt_data(column):  # функция, которая выводит текст каждого курса по отдельному столбцу из DataFrame data
#     promt = f""
#     for course_name in data["Course_name"].values:
#         column_info = data[data['Course_name'] == course_name][column].values[0]
#         if type(column_info) == list:
#             column_info = ", ".join(column_info)
#         promt += f"Для курса с названием '{course_name}' такие данные о '{column}': {column_info} >>>>>>"
#     return promt
#
# print(promt_data('Course_program'))
# s = 'Здравствуйте! Меня зовут Хэнк, я Менеджер - консультант компании Газпромбанк. Рад приветствовать вас в нашем чате! Хотите узнать о наших учебных курсах?'
# print("jfasjlkfj '\\n' fjakljdf \n")
# print('dfjasf')
#
# s = 'Расскажи мне ещё раз план обучения на данном курсе. Только не используй в своём ответе символ  перехода строки "\n".'
#
# print(""" Ghbdtn "cj,frf" z nt, djfljs""")
import sys
print(sys.version)
